...
Examples:
  ...
  python extract_clips.py --num-clips 10 --seed 42 --min-duration 10 --max-duration 30
    ...
    parser.add_argument('--category', type=str,
                        help='Filter by category (e.g., 3d_animation, mixed_content)')
    ...
    parser.add_argument('--output-dir', type=Path, default='data/test_clips',
    ...
    if not metadata_path.exists():
        print("❌ Error: download_metadata.json not found. Run fetch_videos.py first.", file=sys.stderr)
        return 1
    ...
    if not args.no_clean:
        if args.output_dir.exists():
            ...
            if metadata_file.exists():
                ...
                print(f"   Deleted: clip_metadata.json")
    ...
    for i, spec in enumerate(clip_specs, 1):
        ...
        print(f"     Time: {spec['start_time']:.2f}s - {spec['start_time'] + spec['duration']:.2f}s "
        ...
        if not success:
            print(f"     ⚠️  Failed to extract clip", file=sys.stderr)
            continue
