...
class VideoFetcher:
    ...
    def download_video(self, source: dict, resume: bool = True) -> bool:
        ...
        try:
            ...
            if response.status_code == 416:  # Range not satisfiable
                print("  ⚠️  Cannot resume, starting fresh")
                existing_size = 0
                ...
                try:
                    with zipfile.ZipFile(download_path, "r") as zip_ref:
                        ...
                        video_files = [
                            ...
                            if any(
                                f.endswith(ext) for ext in [".mp4", ".mkv", ".mov", ".avi", ".webm"]
                            )
                        ]

                        if not video_files:
                            print("  ❌ No video file found in archive")
                            download_path.unlink()  # Clean up zip
                            return False

                        if len(video_files) > 1:
                            print(
                                f"  ⚠️  Multiple video files in archive, using first one: {video_files[0]}"
                            )
    ...
    def list_sources(self, categories: list[str] | None = None):
        """List all configured video sources."""
        sources = self.sources

        if categories:
            sources = [
                s for s in sources if any(cat in s.get("categories", []) for cat in categories)
            ]
    ...
    def show_info(self, video_id: str):
        ...
        if self._is_download_complete(source):
            ...
            print(
                f"  Size: {meta['size_bytes'] / 1024 / 1024:.1f} MB ({meta['size_bytes']:,} bytes)"
            )
...
Examples:
  ...
  python fetch_videos.py --list-categories
    ...
    parser.add_argument(
        "--config",
        default="config/video_sources.json",
    ...
    action_group.add_argument("--list", action="store_true", help="List all available videos")
    action_group.add_argument(
        "--list-categories", action="store_true", help="List all available categories"
    )
    action_group.add_argument("--info", metavar="ID", help="Show detailed info about a video")
    action_group.add_argument("--all", action="store_true", help="Download all videos")
    ...
    parser.add_argument("--ids", nargs="+", help="Download specific videos by ID")

    # Options
    parser.add_argument("--no-resume", action="store_true", help="Do not resume partial downloads")
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
    ...
    if args.list:
        ...
        results = fetcher.fetch_all(
            categories=args.categories, ids=args.ids, continue_on_error=args.continue_on_error
        )
